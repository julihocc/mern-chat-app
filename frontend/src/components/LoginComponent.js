...
const LoginComponent = () => {
    ...
    const [loginUser, { error }] = useMutation(LOGIN_USER, {
        onError(err) {
            logger.error('LoginComponent Error:', err.message);
            navigate("/dashboard");    
        },
        onCompleted(data) {
          logger.debug("LoginComponent successful. Setting user in Redux store.");
          ...
          client
            ...
            .then(
              ({ data }) => {
                ...
                logger.debug(
                  "client.query data.getCurrentUser._id",
                  data.getCurrentUser._id
                );
              },
              (err) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            logger.error('LoginComponent attempt failed:', err); // Logging error on login attempt
        }
    };
