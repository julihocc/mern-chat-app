...
const ChatRoomViewer = () => {
    ...
    const isLoading = useSelector(state => state.chat.loading || state.currentUser.loading);
    ...
    const handleSendMessage = async (e, senderId, chatRoomId) => {
        e.preventDefault();
        dispatch(sendMessage({ senderId, chatRoomId, body: messageBody, file }));
    };
    ...
    if (!isLoggedIn) return <Typography variant="h4">Please log in to view the chat room</Typography>; // Check the isLoggedIn flag
    ...
    return (
        <Container component={Paper} sx={{height: "90vh", mt: 2, display: "flex", flexDirection: "column", p: 2}}>
            <CssBaseline/>
            <Typography variant="h2" sx={{mb: 2}}>{t("chatRoom")}: {chatRoom.id}</Typography>
            <Typography variant="h5" sx={{mb: 2}}>{t("messages")}</Typography>
            <List>
                {messages && messages.slice(-5).map((message, index) => (
                    <ListItem key={index} sx={{flexDirection: message.senderId === currentUser.id ? "row-reverse" : "row"}}>
                        <ListItemAvatar>
                            <Avatar><PersonIcon/></Avatar>
                        </ListItemAvatar>
                        <ListItemText primary={message.body} secondary={message.senderId}
                                      sx={{textAlign: message.senderId === currentUser.id ? "right" : "left"}}/>
                        {message.fileContent && (
